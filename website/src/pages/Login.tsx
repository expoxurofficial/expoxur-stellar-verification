import { Link } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import Navbar from "@/components/Navbar";
import expoxurLogo from "@/assets/expoxur-logo.png";

export default function Login() {
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <div className="container py-16 max-w-md mx-auto">
        <div className="text-center mb-8">
          <img src={expoxurLogo} alt="Expoxur" className="h-10 w-auto mx-auto mb-4" />
          <h1 className="font-display font-bold text-3xl text-foreground mb-2">Welcome Back</h1>
          <p className="text-muted-foreground text-sm">
            Log in to manage your impact business profile or continue discovering.
          </p>
        </div>

        <div className="bg-card rounded-xl border border-border/80 p-6 flex flex-col gap-5">
          <div>
            <Label className="font-display text-xs font-semibold">Email Address</Label>
            <Input className="mt-1" type="email" placeholder="you@example.com" />
          </div>

          <div>
            <div className="flex items-center justify-between mb-1">
              <Label className="font-display text-xs font-semibold">Password</Label>
              <button className="text-xs text-primary font-display hover:underline">
                Forgot password?
              </button>
            </div>
            <Input type="password" placeholder="Your password" />
          </div>

          <Button className="h-12 bg-primary text-primary-foreground font-display font-bold rounded-xl">
            Log In
          </Button>

          <div className="relative">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-border" />
            </div>
            <div className="relative flex justify-center">
              <span className="bg-card px-3 text-xs text-muted-foreground font-display">or</span>
            </div>
          </div>

          <Button variant="outline" className="h-11 font-display font-medium rounded-xl">
            <svg className="h-4 w-4 mr-2" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Continue with Google
          </Button>
        </div>

        <p className="text-center text-sm text-muted-foreground mt-6">
          Don't have an account?{" "}
          <Link to="/register" className="text-primary font-display font-semibold hover:underline">
            Sign Up
          </Link>
        </p>
      </div>
    </div>
  );
}
